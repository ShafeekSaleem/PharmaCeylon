...
- `lib/role-access.ts` — **mirrors the API's `RolesGuard` exactly**: owner-on-any-branch bypasses all checks, otherwise roles are scoped to the currently selected branch. `rolesForPath()` is the route→roles table (longest-prefix match); `collectUserRoles(user, branchId)` derives the effective role set. If you add a new role-gated route, add it to `rolesForPath()` here **and** add the corresponding `@Roles()` on the API endpoint — this file is UX-only, the API is the real enforcement (see `docs/ROLE_ACCESS_MATRIX.md`).
- `components/role-access/` — `role-page-guard.tsx` (wraps a page and shows `role-access-denied.tsx` if the user lacks access), `role-button.tsx` / `role-link.tsx` (disable/hide controls based on role). Prefer these over ad-hoc `if (role === ...)` checks in page components.
