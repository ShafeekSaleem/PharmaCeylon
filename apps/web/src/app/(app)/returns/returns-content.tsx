...
type ReturnsContentProps = {
  /**
   * Show one kind of return only. Purchasing's Supplier returns tab pins this to `supplier`, so
   * sending stock back to a vendor lives with the orders, deliveries and invoices it belongs to.
   * The Returns page itself stays unpinned until customer returns move into POS refunds.
   */
  lockedType?: "customer" | "supplier";
  /** Rendered under the page header — the Purchasing tab strip, when pinned there. */
  subnav?: ReactNode;
};

export function ReturnsContent({ lockedType, subnav }: ReturnsContentProps = {}) {
  ...
  // Everything below reads this, so a pinned view never counts the other kind of return.
  const typedRows = useMemo(
    () => (lockedType ? returns.rows.filter((row) => row.type === lockedType) : returns.rows),
    [returns.rows, lockedType],
  );

  const [search, setSearch] = useState("");
  const [statusFilter, setStatusFilter] = useState<GoodsReturnStatusFilter>("all");
  const [typeFilter, setTypeFilter] = useState<GoodsReturnTypeFilter>(lockedType ?? "all");
  ...
  const periodStats = useMemo(
    () => periodSummaryFromReturns(typedRows, summaryPeriod),
    [typedRows, summaryPeriod],
  );
  ...
  const alerts = useMemo(() => {
    const pendingApproval = typedRows.filter((r) => r.status === "pending_approval").length;
    const awaiting = typedRows.filter((r) => r.status === "awaiting_logistics").length;
    const inReview = typedRows.filter((r) => r.status === "in_review").length;
    const drafts = typedRows.filter((r) => r.status === "draft").length;
    return [
      {
        key: "approval",
  ...
  return (
    <div className={layoutCss.page}>
      ...
      <PageHeader
        ...
        description={
          lockedType === "supplier"
            ? "Stock sent back to suppliers — expired, damaged or recalled — and the debit note each one raises against what you owe them."
            : "Create customer and supplier returns, approve them, handle pickup/dispatch, and complete stock adjustments."
        }
        actions={
          canWrite ? (
            <ActionButton
            ...
            >
              {lockedType === "supplier" ? "New supplier return" : "New return"}
            </ActionButton>
          ) : null
        }
      />
