...
export type BranchSalesMatrixPoint = { id: string; label: string; transactions: number; avgBasket: number; revenue: number };

type Props = {
  points: BranchSalesMatrixPoint[];
  formatValue: (n: number) => string;
  /** One color per branch — the same map the ranking table/trend chart already use on this page,
   *  so a branch reads as the same color everywhere on it. */
...
/** Transactions × average basket value, bubble size = Net Sales — which branches drive revenue
 *  through footfall/volume vs. through a higher basket, a comparison Branch Profitability's
 *  revenue×margin matrix doesn't answer. Colored per-branch (matching this page's own ranking
 *  table/trend chart palette) rather than by quadrant — unlike revenue×margin, there's no inherent
 *  "good"/"bad" quadrant for transactions×basket, so quadrant color-coding would be manufactured
 *  meaning. Dashed reference lines at the median transactions/basket still divide the plot into
 *  four readable quadrants. Same collision-avoidance + pixel-measured plot pattern as Branch
 *  Profitability / Stock Health / Supplier Performance's matrices (see `resolveBubbleCollisions`'s
 *  doc comment). */
export function BranchSalesMatrix({ points, formatValue, colorFor, onBubbleClick, activeId }: Props) {
  ...
  const sizeOf = (revenue: number) => 12 + Math.sqrt(Math.max(0, revenue) / maxRevenue) * 28;
        ...
        <div className={css.healthMatrixPlotCol}>
          <div className={css.healthMatrixPlot} ref={setPlotEl}>
            ...
            {points.map((p) => {
              ...
              const tooltip = `${p.label} — ${p.transactions.toLocaleString("en-IN")} transactions, ${formatValue(p.avgBasket)} avg. basket, ${formatValue(p.revenue)} net sales`;
              return (
                <div
                  key={p.id}
