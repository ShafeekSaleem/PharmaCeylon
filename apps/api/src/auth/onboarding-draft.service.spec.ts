...
describe("OnboardingDraftService", () => {
  it("prefills Sri Lankan defaults for a verified owner", async () => {
    ...
    const result = await new OnboardingDraftService(prisma as never).get(
      "registration-id",
    );
  ...
  it("merges and persists a resumable draft", async () => {
    const prisma = {
      ownerRegistration: {
        findUnique: jest.fn().mockResolvedValue({
          onboardingDraft: {
            businessName: "Serendib Pharmacy",
            currentStep: 1,
          },
          email: "owner@example.com",
          phone: null,
    ...
    const result = await service.save("registration-id", {
      currentStep: 2,
  ...
  it("never moves resumable progress backwards when an earlier step auto-saves", async () => {
    const prisma = {
      ownerRegistration: {
        findUnique: jest.fn().mockResolvedValue({
          onboardingDraft: {
    ...
    const result = await new OnboardingDraftService(prisma as never).save(
      "registration-id",
      { currentStep: 1, legalName: "Serendib Pharmacy (Pvt) Ltd" },
    );
    expect(result.draft.currentStep).toBe(3);
    expect(result.nextPath).toBe("/onboarding/preferences");
  });
});
